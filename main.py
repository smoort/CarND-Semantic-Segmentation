...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv_transpose_1 = tf.layers.conv2d_transpose(conv_1x1, num_classes, 4, 2, padding = 'same',
                                kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
    conv_transpose_1_with_skip_connection = tf.add(conv_transpose_1, vgg_layer4_out)
    conv_transpose_2 = tf.layers.conv2d_transpose(conv_transpose_1_with_skip_connection, num_classes, 4, 2, padding = 'same',
                                kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        for image, label in get_batches_fn(batch_size):
            ...
            _, train_loss = sess.run([train_op, cross_entropy_loss], feed_dict={input_image: image, correct_label: label, keep_prob: keep_prob, learning_rate: learning_rate})
            print("EPOCH {} ...".format(epoch + 1))
            print("Traing loss = {:.3f}".format(train_loss))
    pass
...
def run():
    ...
    with tf.Session() as sess:
        ...
        """
        loss = sess.run(train_op, feed_dict={ input_image: images, correct_label: labels, keep_prob: 0.5, learning_rate: 0.01})
        img = np.random.rand(8, 160, 576, 3) 
        nn_last_layer_print = tf.Print(nn_last_layer, [tf.shape(nn_last_layer), " nn_last_layer.shape "], summarize=4)
        sess.run(tf.global_variables_initializer())
        sess.run(nn_last_layer_print, feed_dict={input_image: img, keep_prob: 1.0})
        """
